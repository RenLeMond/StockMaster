...
[![Android](https://img.shields.io/badge/Platform-Android%206.0%2B-3DDC84?logo=android)](https://developer.android.com)
...
[![Min SDK](https://img.shields.io/badge/minSdk-23-blue)](app/build.gradle)
[![Target SDK](https://img.shields.io/badge/targetSdk-35-green)](app/build.gradle)
[![License](https://img.shields.io/badge/License-MIT-lightgrey)](LICENSE)
[![Release](https://img.shields.io/github/v/release/RenLeMond/StockMaster?label=Release)](https://github.com/RenLeMond/StockMaster/releases)

</p>

**English** | **中文**

离线优先、零网络依赖的 Android 原生进销存工具。面向闲鱼 / 抖音小店 / 私域 / 档口等个人及微型卖家的全链路方案：扫码出入库、多尺码矩阵、库存预警、毛利核算、流水追溯与 SAF 备份。

<p>

[⬇️ 下载 APK](https://github.com/RenLeMond/StockMaster/releases) · [📸 截图预览](#-screenshots--demo) · [🚀 快速开始](#-quick-start) · [❓ 常见问题](#-faq)
...
`docs/demo/scan-flow.gif` — 建议 10s 内展示：扫码 → 抽屉确认 → 烟花特效 → 自动跳转详情

```md
![Scan Flow](docs/demo/scan-flow.gif)
```
...
| 🔫 | **硬件扫码枪** | 全局 `dispatchKeyEvent` 拦截（`util/ScannerGun.kt:1`），<120ms + Enter 判定，输入框焦点自动放行 |
| 🎆 | **扫码即庆祝** | `ConfettiFireworksEffect` 满屏烟花 + `BeepPlayer` 声触反馈（`util/BeepPlayer.kt:1`），1.6s 自动跳转详情 |
| 📦 | **库存高密度列表** | 3 行 Header（省 155dp+）、模糊搜索（名/SKU/条码/库位）、容量进度条、低库存徽标、`QuickTransactionDialog` 快捷出入 |
| 👕 | **多尺码矩阵** | 预设 S-XXXL / 36-44 + 自定义，单码操作或 `Batch Size Breakdown` 批量配比，总库存自动联动 |
| 🏷️ | **商品全景** | CODE128 高清条码（`util/BarcodeBitmap.kt:1`）、尺码分布柱状图、专属流水时间线、`PhotoViewerDialog` 捏合放大 |
...
2. 在 Android 设备上允许“安装未知应用”后安装
3. 首次扫码授予相机权限即可使用

> 未找到 Release 时可直接用方式二构建 `debug` 包。
...
3. **庆祝与跳转**：成功后满屏烟花，自动跳转商品详情页（`ui/AppRoot.kt:75` 导航栈管理）
...
**导航**：`AppRoot.kt:75` 轻量 `Nav` 栈（`main` Tab + overlay 全屏页），`BackHandler` 统一回退，扫码枪请求去重路由。

**状态**：单 `MainViewModel`（`ui/MainViewModel.kt:56`）+ `StateFlow` + `collectAsStateWithLifecycle`，状态写入收敛主线程。

**持久化**：脏标记 + `Mutex` 合并落盘（`ui/MainViewModel.kt:82`），`Dispatchers.IO` 异步调度，`JsonFileStore` 以 `.tmp` + `renameTo` 原子写入（`data/JsonFileStore.kt:1`）。

**主题**：固定 Light + Edge-to-Edge 透明系统栏（`MainActivity.kt:28`）。
...
定义见 `data/Models.kt:26` / `data/Models.kt:56`：
...
`BackupBundle` 聚合 `items / transactions / categories / locations / images`（Base64 内嵌），跨设备恢复时物化至 `filesDir/images/` 并重映射路径（`ui/MainViewModel.kt:488`）。
...
| UI | Jetpack Compose BOM 2025.02.00 + Material3 · Edge-to-Edge |
| 架构 | 单 `MainViewModel` (`ui/MainViewModel.kt:56`) + `StateFlow` + `collectAsStateWithLifecycle` |
| 相机 / 扫码 | CameraX 1.4.1 (core/camera2/lifecycle/view) + ML Kit 17.3.0 + ZXing 3.5.3 |
| 图片 | Coil 2.7.0（仅 `file://` / data URI）+ ExifInterface + `ImageUtils` 私有目录解耦 |
| 序列化 / 存储 | `kotlinx-serialization-json` 1.8.0 · `JsonFileStore` 原子写 (`data/JsonFileStore.kt:1`) |
| 并发 | `kotlinx.coroutines` + `Mutex` 脏标记合并持久化 (`ui/MainViewModel.kt:82`) |
| 其他 | `core-ktx` 1.15.0 · `activity-compose` 1.9.3 · `lifecycle-viewmodel-compose` 2.8.7 · desugar 2.1.5 |

---

## 🔒 Privacy & Offline

`AndroidManifest.xml:4` 仅声明 `android.permission.CAMERA`（`required=false`），**未申请 `INTERNET`**：

- ML Kit 内置离线模型，Coil 仅加载本地 `file://` / data URI
- 数据 100% 在 `context.filesDir`（`items.json` / `transactions.json` / `categories.json` / `locations.json` + `images/img_*.jpg`），`allowBackup=false`
- 持久化 `.tmp` + `renameTo` 原子重命名，`Dispatchers.IO` + `Mutex` 串行互斥，扫码枪高频连扫零竞态

---

## 💾 Backup & CSV

入口：`设置` 页（`ui/screens/SettingsScreen.kt:1`），经 SAF（`CreateDocument` / `OpenDocument`）由用户自主触发，无后台静默备份。

| 操作 | 格式 | 编码 |
|---|---|---|
| 全量备份 | JSON (`BackupBundle`) | UTF-8，含图片 Base64 内嵌 `images` |
| 全量恢复 | JSON | 校验 `version`/`appName`，物化图片到私有目录 |
| 商品档案导出/导入 | CSV | UTF-8 BOM |
| 流水导出/导入 | CSV | UTF-8 BOM |

- 商品导入以 `SKU`/`条码` 为键合并覆盖
- 流水导入按 `id` 去重并按 `SKU` 回填 `itemId`
...
**Q1 扫码枪扫不上 / 与输入法冲突？**
`ScannerGun` 仅当按键间隔 <120ms 且以 Enter 结尾时判定为扫码（`util/ScannerGun.kt:1`），且在文本框焦点时自动放行。请在非编辑状态下扫码，或检查扫码枪是否配置为“回车后缀”模式。

**Q2 备份后跨设备恢复图片丢失？**
旧格式备份无 `images` 字段属正常；新格式已内嵌 Base64，恢复时物化至 `filesDir/images/`（`ui/MainViewModel.kt:488`）。若仍不显示，请确认备份文件为最新版本导出的 JSON。

**Q3 数据文件在哪？如何手动清理？**
私有目录 `filesDir` 下 `items.json` 等 + `images/`。卸载即清除；应用内“清空全部数据”会重置分类/库位为预设并递归删除 `images/`。请先在设置页导出备份。
...
- 分支：`feature/*` / `fix/*` → PR 到 `main`
- 提交信息：`feat/fix/docs/refactor: 摘要`（中文亦可，动词开头）
- 代码风格：遵循官方 Kotlin 风格，Compose 遵循 Material3 规范

> 计划补充 `CONTRIBUTING.md` / `CODE_OF_CONDUCT.md` / `.github/ISSUE_TEMPLATE`，欢迎贡献。

---

## 📄 License & Acknowledgements

MIT — 详见 [LICENSE](LICENSE)。未另行声明时，第三方依赖遵循各自许可证。
